function gsdo_apriori_map, index, data, f_var, windowed = windowed, n_iter = n_iter, w_param = w_param
    ...
    h_q = GSDO_HIST2D(/STRUCT,   $
          ...
          MIN_Y = 0, MAX_Y = 4.5, N_Y = b, /nonorm)
    ...
    ;;; during division
    hq_conv = CONVOL(float(h_q.hist)/n_elements(ix_q), krn_hist, /EDGE_TRUNC) 
    hall_conv = CONVOL(float(h_all.hist)/n_elements(data[0,0,*]), krn_hist, /EDGE_TRUNC)  
