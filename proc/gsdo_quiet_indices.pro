function gsdo_quiet_indices, index, cube1, cube2,  QUANT=quant,   $
  ...
  if _p then begin
   ...
   ; stop
    ...
    UTPLOT, t - t0, var_indic, t0,  ps=-1, TITLE='GSDO_QUIET_INDICES',    $
...
;  return, where(var_indic le thr)
	...
	q_idx = s[0:round(n_elements(s)*quant)]
	 ...
	 if _p then begin
		...
		OUTPLOT, (t - t0)[q_idx], var_indic[q_idx], t0,  ps=6, symsize=3
